...
def send_log(mall_name, keywords, results_dict):
    # Airtable 로깅 기능 제거됨 - 로컬 로그만 출력
    print(f"검색 완료: {mall_name}, 키워드: {', '.join(keywords)}")
    print(f"결과 개수: {len(results_dict)}")
...
class RankCheckerApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("네이버 순위 확인기 (by ORYNE)")
    ...
    def setup_ui(self):
        ...
        footer = QLabel("ⓒ 2025 ORYNE. 무단 복제 및 배포 금지. All rights reserved.")
